...
def generate_signals(file, h0, depth, start, h1_timestamp, l1_timestamp, frequencies, draw_signal, seed):
    ...
    def preprocess_amplitude(x):        
        ...
        x = np.asarray(np.squeeze(x[:,:4096]) , np.complex128)
        ...
        print("POS, scale", POS, scale, scale / EXP.astype(scale.dtype) ** 2, x.shape)
        x = x / (scale / EXP.astype(scale.dtype) ** 2)
        
        print("SHAPE (2) \n\n", x.shape, "\n\n")
        # x is currently of shape (360, 4096)
        x = x.reshape((360, 512,8))
...
def generate_signals_for_file(file, seed=42):
    ...
    h0 = 10. * np.random.uniform(0.1, 10) # 10**np.random.uniform(-25, -22)
