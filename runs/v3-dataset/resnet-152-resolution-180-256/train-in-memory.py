...
def train(depth_min):
    ...
    df = pd.read_csv("/media/viktor/T7/gravitational-waves-kaggle-2022/datasets/dataset-v3/data.csv")
    ...
    df_test = pd.read_csv("/media/viktor/T7/gravitational-waves-kaggle-2022/kaggle-data/preprocessed-4/train-preprocessed.csv")
    df_test["preprocessed_filename"] = df_test["preprocessed_filename"].apply(lambda x: x.replace("/media/viktor/T7/gravitational-waves-kaggle-2022", "/media/viktor/T7/gravitational-waves-kaggle-2022/kaggle-data/preprocessed-4"))
    # rename "preprocessed_filename" to "file" and "target" to "label"
    df_test = df_test.rename(columns={"preprocessed_filename": "file", "target": "label"})
    ...
    random_indices = list(range(len(train_files)))
    ...
    random_indices = list(range(len(test_files)))
    random.shuffle(random_indices)
    test_files = [test_files[i] for i in random_indices]
    test_labels = [test_labels[i] for i in random_indices]
    ...
    print("[INFO] Creating tf.data.Dataset objects...")
    ...
    model = largeKernelDenoiser() # create_model()
    model.summary()
    # exit()
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=5.E-06),
                    ...
                    loss=  tf.keras.losses.BinaryFocalCrossentropy(gamma=2), # tf.keras.losses.BinaryFocalCrossentropy(gamma=4),
    ...
    # set the learning rate to 1.E-04
    model.optimizer.lr = 1.E-04
    ...
    # make a custom callback for adjusting the learning rate
    ...
    class WarmupCallback(tf.keras.callbacks.Callback):
        def __init__(self, warmup_epochs=128):
        ...
        def on_epoch_begin(self, batch, logs=None):
            ...
            if self.epochs < self.warmup_epochs:
                lr = self.model.optimizer.lr * (self.epochs / self.warmup_epochs)
    ...
    history = model.fit(train_dataset, 
            ...
            callbacks=[
                WandbCallback(save_model=False), 
                SaveBest(),
                # WarmupCallback(warmup_epochs=64),
                ]
            )
